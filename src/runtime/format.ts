...
function toDate(input: string | number | Date | undefined): Date {
...
function normalizeOffset(utcOffset: number | undefined): number {
  const numeric = Number(utcOffset);
  if (!Number.isFinite(numeric)) return 0;
  return numeric;
}

export function formatOffsetLabel(utcOffset: number | undefined): string {
